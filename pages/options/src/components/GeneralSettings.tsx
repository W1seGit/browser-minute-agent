      ...
      <div
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <h3 className={`text-base font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                {t('options_general_aiSpaceTabAccess')}
              </h3>
              <p className={`text-sm font-normal ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                {t('options_general_aiSpaceTabAccess_desc')}
              </p>
            </div>
            <label htmlFor="aiSpaceTabAccess" className="sr-only">
              {t('options_general_aiSpaceTabAccess')}
            </label>
            <select
              id="aiSpaceTabAccess"
              value={settings.aiSpaceTabAccess}
              onChange={e =>
                updateSetting('aiSpaceTabAccess', e.target.value as GeneralSettingsConfig['aiSpaceTabAccess'])
              }
              className={`w-40 rounded-md border ${isDarkMode ? 'border-slate-600 bg-slate-700 text-gray-200' : 'border-gray-300 bg-white text-gray-700'} px-3 py-2`}>
              <option value="ask">{t('options_general_aiSpaceTabAccess_ask')}</option>
              <option value="alwaysAllow">{t('options_general_aiSpaceTabAccess_alwaysAllow')}</option>
              <option value="alwaysDeny">{t('options_general_aiSpaceTabAccess_alwaysDeny')}</option>
            </select>
          </div>
        </div>
      </div>
